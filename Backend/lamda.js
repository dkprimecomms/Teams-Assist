// backend/index.mjs (or whatever your Lambda entry file is)
...
function normalizeEvent(ev, nowUtcMs) {
  ...
  return {
    ...
    // ✅ Recurrence support
    eventType: ev?.type || null,          // <— IMPORTANT: use ev.type (NOT "eventType" in $select)
    seriesMasterId: ev?.seriesMasterId || null,
    iCalUId: ev?.iCalUId || null,
    recurrence: ev?.recurrence || null,
...
export const handler = async (event) => {
  try {
    ...
    // POST /graph/events
    // Body: { token, startISO, endISO, status? }
    if (method === "POST" && path === "/graph/events") {
      const body = await readJsonBody(event);
      if (!body) return json(event, 400, { ok: false, error: "Invalid JSON body" });
      if (!body.token) return json(event, 400, { ok: false, error: "Missing token" });

      const { startISO, endISO, status: desiredStatus } = body;
      if (!startISO || !endISO) return json(event, 400, { ok: false, error: "Missing startISO/endISO" });
      ...
      try {
        ...
        // ✅ IMPORTANT:
        // - Use "type" (NOT "eventType") to avoid 400 parse errors.
        // - Include iCalUId for recurrence enrichment fallback in Teams.
        const query =
          ...
          [
            ...
            "seriesMasterId", // ✅ helps master lookup
            "iCalUId",        // ✅ fallback when seriesMasterId missing
            "recurrence",     // ✅ may be null on occurrences (expected)
            "showAs",
            "createdDateTime",
        ...
        const data = await graphGet(graphToken, query);
        const nowUtcMs = Date.now();
        let normalized = (data.value || []).map((ev) => normalizeEvent(ev, nowUtcMs));

        // ✅ Enrich recurrence for occurrences/exceptions:
        // 1) try seriesMasterId
        // 2) fallback to iCalUId -> find seriesMaster -> recurrence
        ...
        const filtered = desiredStatus ? normalized.filter((m) => m.status === desiredStatus) : normalized;
        return json(event, 200, { ok: true, value: filtered });
      } catch (e) {
        return json(event, 500, { ok: false, error: String(e?.message || e) });
      }
    }
